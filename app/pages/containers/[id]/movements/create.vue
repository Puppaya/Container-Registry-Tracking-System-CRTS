...
function onSuccess() {
  router.push(`/containers/${containerId.value}/movements`)
}
...
<template>
  <UDashboardPanel id="container-movement-create" grow>
    <template #header>
      <UDashboardNavbar :title="container ? `${t('common.recordMovement')} — ${container.containerNumber}` : t('common.recordMovement')">
        <template #leading>
          <div class="flex items-center gap-2">
            ...
            <UButton
              ...
              color="neutral"
              variant="ghost"
              :to="`/containers/${containerId}/movements`"
            />
          </div>
        </template>
      </UDashboardNavbar>
